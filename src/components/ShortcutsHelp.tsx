...
export function ShortcutsHelp({
  ...
  const handleSave = () => {
    ...
    try {
      ...
      const updatedShortcuts = localShortcuts.reduce(
        (acc, s) => {
          const id = getShortcutId(s.description);
          if (id) acc[id] = s.key;
          return acc;
    ...
    if (!editingId) {
      const handleNavigation = (e: KeyboardEvent) => {
        if (e.key === "ArrowDown") {
          ...
          setSelectedIndex(
            (prev) =>
              (prev - 1 + editableShortcuts.length) % editableShortcuts.length,
          );
        } else if (e.key === "Enter") {
          e.preventDefault();
      ...
      return () =>
        window.removeEventListener("keydown", handleNavigation, true);
    }
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (!["Meta", "Control", "Shift", "Alt"].includes(mainKey)) {
        ...
        setLocalShortcuts((prevShortcuts) =>
          prevShortcuts.map((s) => {
            const id = getShortcutId(s.description);
            if (id === editingId) {
  ...
  const groupedShortcuts = localShortcuts.reduce(
    (acc, shortcut) => {
  ...
  return (
    ...
    >
      <div
        className="bg-black/90 border border-white/10 rounded-xl w-full max-w-3xl shadow-2xl flex flex-col animate-in fade-in zoom-in duration-150 overflow-hidden ring-1 ring-white/5 max-h-[85vh]"
              ...
              <div key={group}>
                ...
                <div className="space-y-2">
                  {groupShortcuts.map((shortcut, idx) => {
                    ...
                    const globalIndex = editableShortcuts.findIndex(
                      (s) => s.description === shortcut.description,
                    );
                    const isSelected =
                      !editingId && isEditable && globalIndex === selectedIndex;
