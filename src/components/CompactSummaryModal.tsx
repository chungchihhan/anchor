...
export function CompactSummaryModal({
  ...
  return (
    <>
      ...
      >
        <div
          className="bg-black/90 border border-white/10 rounded-xl w-full max-w-4xl max-h-[85vh] shadow-2xl flex flex-col animate-in fade-in zoom-in duration-150 overflow-hidden ring-1 ring-white/5"
        ...
        >
          ...
          <div className="flex items-center justify-between p-4 border-b border-white/10 bg-white/5 shrink-0">
            ...
            <div className="flex items-center gap-3">
              <div className="text-[10px] text-white/40 border border-white/10 px-1.5 py-0.5 rounded font-mono">
                ESC to {hasChanges ? 'cancel' : 'close'} • {hasChanges ? 'Save to apply changes' : 'Edit to modify'}
              </div>
              {hasChanges && (
                <button
                  onClick={handleSave}
          ...
          <div className="flex-1 overflow-y-auto p-6 custom-scrollbar bg-black/40">
            <textarea
              ...
              className="w-full h-full min-h-[400px] bg-white/5 border border-white/10 rounded-lg p-4 text-white font-mono text-sm resize-none focus:outline-none focus:border-cyan-500/50 focus:bg-white/10 focus:ring-2 focus:ring-cyan-400/30 transition-all"
      ...
      {/* Confirmation Dialog */}
      {showConfirmDialog && (
        <div className="fixed inset-0 bg-black/60 z-[60] flex items-center justify-center p-4 transition-all">
          <div 
            className="bg-black/90 border border-white/10 rounded-xl p-6 max-w-md shadow-2xl animate-in fade-in zoom-in duration-150 ring-1 ring-white/5"
            onClick={(e) => e.stopPropagation()}
          >
